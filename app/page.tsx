...
export default function Page() {
  ...
  // Bezpečné hodnoty pre výpočet (prázdne / neplatné polia = 0).
  ...
  return (
    <div className="min-h-svh bg-background">
      <header className="border-b-2 border-primary bg-primary text-primary-foreground print:hidden">
        <div className="mx-auto flex max-w-6xl flex-col gap-1 px-4 py-4 md:px-6">
          <div className="flex items-center gap-3">
            <span className="rounded bg-primary-foreground px-2 py-0.5 font-mono text-xs font-bold uppercase tracking-widest text-primary">
              Promosteel
            </span>
          ...
          <h1 className="text-balance text-2xl font-bold md:text-3xl">
            Konfigurátor hliníkovej bránky
          </h1>
          <p className="text-sm text-primary-foreground/70">
            Jednokrídlová bránka s latovou výplňou — výpočet materiálu na obhliadke
          </p>
        </div>
      </header>
      ...
      <main className="mx-auto grid max-w-6xl grid-cols-1 gap-8 px-4 py-8 md:px-6 lg:grid-cols-2 print:block print:max-w-none print:px-0 print:py-0">
        {/* Ľavý stĺpec: formulár (skrytý pri tlači) */}
        <section aria-label="Rozmery bránky" className="print:hidden">
          <h2 className="mb-4 text-lg font-bold text-foreground">Rozmery</h2>
          <GateForm vstup={vstup} onChange={setVstup} />
        </section>
        ...
        <section aria-label="Náhľad a výsledky" className="flex flex-col gap-6">
          <div>
            ...
            <div className="rounded-md border border-border bg-muted p-4 print:border-none print:bg-white print:p-0">
              <GatePreview vstup={bezpecnyVstup} vysledok={vysledok} />
            </div>
          </div>
