...
import { SIRKY_LAMIEL_MM, POVRCHY, type SirkaLamely, type PovrchId, type Orientacia } from "@/lib/gate-config"
...
export function GateForm({ vstup, onChange }: Props) {
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Osadenie lamiel – vertikálne / horizontálne */}
      <div>
        <span className="mb-2 block text-sm font-semibold uppercase tracking-wide text-muted-foreground">
          Osadenie lamiel
        </span>
          ...
          {(
            ...
            return (
              <button
                key={o.id}
                type="button"
                onClick={() => onChange({ ...vstup, orientacia: o.id as Orientacia })}
                aria-pressed={active}
                className={
                  "flex items-center justify-center gap-3 rounded-md border-2 py-4 text-base font-bold transition-colors " +
                  (active
      ...
      {/* Šírka lamely – veľké tabletovo-ovládateľné tlačidlá */}
      <div>
        <span className="mb-2 block text-sm font-semibold uppercase tracking-wide text-muted-foreground">
          Šírka lamely
        </span>
          ...
          {SIRKY_LAMIEL_MM.map((s) => {
            ...
            return (
              <button
                key={s}
                type="button"
                onClick={() => onChange({ ...vstup, sirkaLamely: s as SirkaLamely })}
                aria-pressed={active}
                className={
                  "rounded-md border-2 py-4 font-mono text-xl font-bold tabular-nums transition-colors " +
                  (active
                    ? "border-primary bg-primary text-primary-foreground"
                    : "border-input bg-background text-foreground hover:border-primary")
                }
      ...
      <NumberField
        label="Medzera medzi lamelami"
        value={vstup.medzera}
        suffix="mm"
        hint="štandardná medzera pre estetický vzhľad"
        onChange={(v) => onChange({ ...vstup, medzera: v })}
      />

      {/* Vzhľad / povrchová úprava lamely */}
      <div>
        <span className="mb-2 block text-sm font-semibold uppercase tracking-wide text-muted-foreground">
          Vzhľad lamely
        </span>
          ...
          {POVRCHY.map((p) => {
            ...
            return (
              <button
                key={p.id}
                type="button"
                onClick={() => onChange({ ...vstup, povrch: p.id as PovrchId })}
                aria-pressed={active}
                title={p.nazov}
                className={
                  "flex flex-col items-center gap-2 rounded-md border-2 p-2 text-center transition-colors " +
                  (active
              ...
              >
                <span
                  className="h-10 w-full rounded ring-1 ring-inset ring-black/10"
                  style={
                    p.drevo
                      ? {
                          backgroundImage: `repeating-linear-gradient(90deg, ${p.farba}, ${p.farba} 3px, rgba(0,0,0,0.14) 3px, rgba(0,0,0,0.14) 4px), linear-gradient(180deg, rgba(255,255,255,0.15), rgba(0,0,0,0.15))`,
                          backgroundColor: p.farba,
                        }
                      : { backgroundColor: p.farba }
                  }
                  aria-hidden="true"
